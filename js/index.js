...
var wizardSlider = function(el) {
	...
	$wizardImgs.on('beforeChange', function(event, slick, currentSlide, nextSlide) {
...
(function() {
	...
	document.addEventListener('keydown', function(e) {
		...
		if (dimClassNames.indexOf('is-active') > -1 && esc === e.keyCode) onPopupCloseHandler();
	});
})();
...
(function($, global, TimelineMax, TweenMax) {
	...
	var expandAnim = function() {
		...
		for (var i = 0; i < $dashboardImgs.length; i += 1) {
			tl.add(extendPic($dashboardImgs.eq(i))).add(restorePic($dashboardImgs.eq(i)));
		}
	};
